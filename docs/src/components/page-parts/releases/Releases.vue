...
export default {
  ...
  setup () {
    ...
    const currentPackage = ref('Vue Laminate')
    const versions = ref({})
    ...
    function queryReleases (page = 1) {
      ...
      xhrQuasar.addEventListener('load', function () {
        ...
        if (!releases || releases.length === 0 || releases.message) {
          error.value = true
          return
        }
        ...
        // sort by date
        packages.value.Releases.sort((a, b) => {
      ...
      xhrQuasar.open('GET', `https://api.github.com/repos/hawkeye64/vue-laminate/releases?page=${ page }&per_page=100`)
      xhrQuasar.send()
    }
