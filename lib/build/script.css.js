...
const themeFiles = glob.sync(path.join(__dirname, '../src/themes/*.scss'))
themeFiles.map(file => {
  const baseName = path.basename(file, '.scss')
  generate(file, `dist/themes/${baseName}`)
})

Promise
  .all([
    generate('src/index.scss', `dist/index`),
    generate('src/auto-light-dark.scss', `dist/auto-light-dark`),
    generate('src/quasar-shim.scss', `dist/quasar-shim`)
  ])
  .catch(e => {
    console.error(e)
    process.exit(1)
